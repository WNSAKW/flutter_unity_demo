...
#unity-mobile-warning { position: absolute; left: 50%; top: 5%; transform: translate(-50%); background: white; padding: 10px; display: none }
        ...
        */
        ...
        private static void MitigateAddressableErrors()
        {
            try
            {
                ...
                // 1. Close Addressables Report Window if open
                var windowType = FindType("UnityEditor.AddressableAssets.GUI.BuildReportWindow");
                if (windowType != null)
                {
                    var openWindows = UnityEngine.Resources.FindObjectsOfTypeAll(windowType);
                    foreach (var w in openWindows)
                    {
                        if (w is UnityEditor.EditorWindow editorWindow)
                        {
                            editorWindow.Close();
                            UnityEngine.Debug.Log("[Antigravity] Closed Addressables BuildReportWindow to avoid layout error.");
                        }
                    }
                }

                // 2. Disable GenerateBuildLayout in ProjectConfigData
                var projectConfigDataType = FindType("UnityEditor.AddressableAssets.Settings.ProjectConfigData");
                if (projectConfigDataType != null)
                {
                    var prop = projectConfigDataType.GetProperty("GenerateBuildLayout", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
                    if (prop != null)
                    {
                        prop.SetValue(null, false);
                        UnityEngine.Debug.Log("[Antigravity] Disabled Addressables GenerateBuildLayout (property) to prevent build failure.");
                    }
                    else
                    {
                        var field = projectConfigDataType.GetField("GenerateBuildLayout", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
                        if (field != null)
                        {
                            field.SetValue(null, false);
                            UnityEngine.Debug.Log("[Antigravity] Disabled Addressables GenerateBuildLayout (field) to prevent build failure.");
                        }
                    }
                }

                // 3. Clear corrupted buildlayout and build report cache
                string projectPath = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
                string addressablesCachePath = Path.Combine(projectPath, "Library", "com.unity.addressables");
                if (Directory.Exists(addressablesCachePath))
                {
                    string layoutFile = Path.Combine(addressablesCachePath, "buildlayout.json");
                    if (File.Exists(layoutFile))
                    {
                        try
                        {
                            File.Delete(layoutFile);
                            UnityEngine.Debug.Log("[Antigravity] Deleted potentially corrupted buildlayout.json in Library.");
                        }
                        catch (System.Exception fileEx)
                        {
                            UnityEngine.Debug.LogWarning("[Antigravity] Could not delete buildlayout.json: " + fileEx.Message);
                        }
                    }
                    
                    string reportsDir = Path.Combine(addressablesCachePath, "BuildReports");
                    if (Directory.Exists(reportsDir))
                    {
                        try
                        {
                            Directory.Delete(reportsDir, true);
                            UnityEngine.Debug.Log("[Antigravity] Deleted potentially corrupted BuildReports cache directory in Library.");
                        }
                        catch (System.Exception dirEx)
                        {
                            UnityEngine.Debug.LogWarning("[Antigravity] Could not delete BuildReports directory: " + dirEx.Message);
                        }
                    }
                }
            }
            ...
            {
                UnityEngine.Debug.LogWarning("[Antigravity] Addressables mitigation failed: " + ex.Message);
            }
        }
